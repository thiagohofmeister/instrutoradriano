  ...
  const openDropdown = useCallback((): void => {
    DropdownButton.current?.measure(
      (_fx: number, _fy: number, _w: number, h: number, _px: number, py: number) => {
        let top = py + h + 2
        const flatListH = FlatListRef.current.getNativeScrollRef().scrollHeight

        if (top + flatListH > Dimensions.get('window').height) {
          top -= flatListH + 40
        }
    ...
    setVisible(true)
  }, [DropdownButton, FlatListRef, setDropdownTop])
